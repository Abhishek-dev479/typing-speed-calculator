let dataArray = [
    ...
    "In 1947, Cho Hong-jai, the Hyosung groups founder, jointly invested in a new company called Samsung Mulsan Gongsa, or the Samsung Trading Corporation, with the Samsungs founder Lee Byung-chul. The trading firm grew to become the present-day Samsung C&T Corporation. After a few years, Cho and Lee separated due to differences in management style. Cho wanted a 30 equity share. Samsung Group was separated into Samsung Group and Hyosung Group, Hankook Tire and other businesses"
]
...
function updateCursor(index){
    // Array.from(letters).forEach((e, i) => {
    //     if(i != index) e.classList.remove('cursor');
    //     else e.classList.add('cursor');
    // })
    if(l) l.classList.remove('cursor');
    letters[index].classList.add('cursor');
    l = letters[index];
}
